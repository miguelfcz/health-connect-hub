  ...
  const fetchAgendamentos = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          profissional:profiles!agendamentos_profissional_id_fkey(nome, especialidade)
        `)
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold text-foreground">Meus Agendamentos</h2>
          <p className="text-muted-foreground">Gerencie suas consultas</p>
        </div>
        <Button onClick={() => navigate("/agendar")} className="shadow-hover">
          ...
          {agendamentos.map((agendamento) => (
            <Card key={agendamento.id} className="shadow-soft hover:shadow-hover transition-all">
              <CardHeader>
                ...
                <CardDescription>{agendamento.profissional.especialidade || "Profissional de Saúde"}</CardDescription>
              </CardHeader>
